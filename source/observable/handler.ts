...
export function handler<T, R = T>(operator: OperatorFunction<T, R>): ((value: T) => void) & ObservableInput<R>;
export function handler<T, R>(operator?: OperatorFunction<T, R>): ((value: T) => void) & ObservableInput<T | R> {
