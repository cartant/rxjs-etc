...
class InexorablyOperator<T> implements Operator<T, T> {
    constructor(private callback: (notification: Notification<T> | undefined) => void) {
    }
...
class InexorablySubscriber<T> extends Subscriber<T> {
    ...
    constructor(destination: Subscriber<T>, callback: (notification: Notification<T> | undefined) => void) {
        super(destination);
