...
import { concat, from, MonoTypeOperatorFunction, ObservableInput, of } from "rxjs";
...
export function switchTap<T>(next: (value: T) => ObservableInput<any>): MonoTypeOperatorFunction<T> {
    return source => source.pipe(
        publish(published => published.pipe(
            concatMap(value => concat(
